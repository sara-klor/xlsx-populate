...
class Workbook {
  ...
  */
    ...
    sheet(sheetNameOrIndex) {
        ...
        var i = this._sheetsNameIndex[sheetNameOrIndex.toString()]
    ...
    _initAsync(data, opts) {
        ...
        return externals.Promise.resolve()
            ...
            .then(nodes => {
                ...
                this._sheetsNode.children.map((sheetIdNode, index) => {
                    this._sheetsNameIndex[sheetIdNode.attributes.name] = index;
                ...
                var handler = {
                    get: function (sheets, sheet) {
                        if (!sheet || !sheets)
                            return
                        if (!(sheet in sheets)) {
                            return workbook._parseNodesAsync(["xl/worksheets/sheet".concat(parseInt(sheet) + 1, ".xml"), "xl/worksheets/_rels/sheet".concat(parseInt(sheet) + 1, ".xml.rels")])
                                .then((nodes) => {
                                    var currentSheet = workbook._sheetsNode.children[sheet];
                                    var sheetNode = nodes[0];
                                    var sheetRelationshipsNode = nodes[1];
                                    sheets[sheet] = new Sheet(workbook, currentSheet, sheetNode, sheetRelationshipsNode);
                                    return sheets[sheet]
                                })
                        } else {
                            return sheets[sheet]
                ...
                this._sheets = new Proxy([], handler);
            })
            .then(() => this);
    }
